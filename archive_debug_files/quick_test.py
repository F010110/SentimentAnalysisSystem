...
try:
    print("\n✓ 导入 BERT_main_aggressive...")
    from BERT_main_aggressive import print_config, SentimentDataset
    print(f"  BERT_main_aggressive 可用")
except Exception as e:
    print(f"  ✗ 失败: {e}")
    sys.exit(1)
...
try:
    ...
    df_train['attitude_id'] = df_train['attitude'].map(label_to_id)
...
try:
    print("\n✓ 创建 Dataset...")
    # 使用已映射的标签列
    dataset = SentimentDataset(df_train.head(10), tokenizer, label_column='attitude_id')
...
print("="*60)
print("\n运行激进修复训练:")
print("  python BERT_main_aggressive.py")
print("\n或者先诊断:")
print("  python diagnose_bert.py")
